...
#define SIF_LOW                    			0      	//低电平
#define SIF_HIGH                    		1      	//高电平

#define SIF_REV_SYNC_L_TIME_NUM         500     //同步信号低电平时间：30ms = 30000us / 50us = 600
...
#define SIF_REV_HALF_LOGIC_CYCLE_MIN    27      //一个逻辑周期的1/2最小时间：1.5ms-150us = 1350us / 50us = 27
#define SIF_REV_HALF_LOGIC_CYCLE_MAX    33      //一个逻辑周期的1/2最大时间：1.5ms+150us = 1650us / 50us = 33

#define SIF_REV_END_SIGNAL_TIME_NUM     60      //结束信号电平时间：5ms低电平 + Nms高电平，实际检测5ms低电平就行，一帧数据发送完成后检测5ms低电平就代表完成了，不发数据的时候上拉电阻拉高了

#define SIF_REV_BIT_NUM                 8       //接收的bit位个数，1字节=8bit

#define SIF_REV_DATA_NUM                12      //接收的数据个数

#define SIF_SEND_SYNC_L_TIME_NUM        600     //同步信号低电平时间：30ms = 30000us / 50us = 600
#define SIF_SEND_SYNC_H_TIME_NUM        10      //同步信号高电平时间：500us = 500us / 50us = 10
#define SIF_SEND_SHORT_TIME_NUM         10      //一个逻辑周期中短的时间：500us = 500us / 50us = 10
#define SIF_SEND_LONG_TIME_NUM          20      //一个逻辑周期中长的时间：1ms = 1000us / 50us = 20

#define SIF_SEND_BIT_NUM                8       //发送的bit位个数，1字节=8bit

#define SIF_SEND_DATA_MAX_NUM           12      //发送的数据最大个数
...
typedef enum
{
	SIF_REV_STATE_INITIAL = 0,    //初始状态，等待接收同步信号
	SIF_REV_STATE_SYNC_L = 1,    //接收同步低电平信号状态
	SIF_REV_STATE_SYNC_H = 2,    //接收同步高电平信号状态
	SIF_REV_STATE_REV_BIT = 3,    //读取数据码电平状态
	SIF_REV_STATE_BUILD_DATA = 4,    //构建字节数据
	SIF_REV_STATE_END_SIGNAL = 5,    //接收结束电平信号状态
	SIF_REV_STATE_IDLE_SIGNAL = 6,
	SIF_REV_STATE_RESTART = 7,     //接收过程出错重新接收状态
	SIF_REV_STATE_STOP = 8
} SIF_REV_STATE_T;   //接收数据状态枚举

typedef enum
{
	SIF_SEND_STATE_INITIAL = 0,    //初始状态，等待发送数据标志
	SIF_SEND_STATE_SYNC_L = 1,    //发送同步低电平信号状态
	SIF_SEND_STATE_SYNC_H = 2,    //发送同步高电平信号状态
	SIF_SEND_STATE_GET_BIT = 3,    //获取数据位
	SIF_SEND_STATE_BIT_L = 4,    //发送数据码低电平信号状态
	SIF_SEND_STATE_BIT_H = 5,    //发送数据码高电平信号状态
	SIF_SEND_STATE_CHECK_BIT = 6,    //检查数据位状态
	SIF_SEND_STATE_END_SIGNAL = 7,    //发送结束电平信号状态
