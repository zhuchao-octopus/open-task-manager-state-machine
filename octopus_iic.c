...
void i2c_init(void) {
    ...
    hal_gpio_set_mode(GPIO_I2C_GROUP_B, GPIO_I2C_SCL_PIN, GPIO_MODE_OUTPUT);
    hal_gpio_set_mode(GPIO_I2C_GROUP_B, GPIO_I2C_SDA_PIN, GPIO_MODE_OUTPUT);

    iic_scl_high(); // Set SCL to high initially
    iic_sda_high(); // Set SDA to high initially
    delay_us(5);    // Small delay to ensure lines are stable
...
uint8_t i2c_wait_ack(void) {
    ...
    while (read_sda_()) { // Wait for SDA to go low (ACK)
        retry++;
        if (retry > 200) { // Timeout after 200 retries (indicating no ACK)
...
void i2c_send_byte(uint8_t data) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (data & 0x80) { // Send the most significant bit first
            iic_sda_high();
        } else {
            iic_sda_low();
        }
...
uint8_t i2c_read_byte(uint8_t ack) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        data <<= 1;
        if (read_sda_()) { // Read the bit from SDA
            data |= 0x01;
        }
