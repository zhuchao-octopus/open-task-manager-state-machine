...
uint32_t decode_version_from_filename(const char* filepath) {
    ...
    if (!filename) filename = strrchr(filepath, '\\'); // For Windows-style paths
    ...
    if (!underscore || strlen(underscore) < 4) return 0;
    ...
    for (int i = 0; i < 12; ++i) {
        if (!isdigit((unsigned char)datetime_str[i])) return 0;
    }
    ...
    for (int i = 0; i < 3; ++i) {
        if (!isdigit((unsigned char)version_str[i])) return 0;
    }
...
bool validate_hex_crc(const char* line) {
    ...
    if (len < 11 || line[0] != ':') return false;  // Must start with ":"

    // Skip the starting ":" character
    for (size_t i = 1; i < len - 2; i += 2) {
        char byte_str[3] = {line[i], line[i + 1], '\0'};
        uint8_t byte = (uint8_t)strtol(byte_str, NULL, 16);  // Convert 2 hex chars to 1 byte
        sum += byte;
    }

    // The last two characters in the line represent the checksum (CRC)
    char checksum_str[3] = {line[len - 2], line[len - 1], '\0'};
    uint8_t checksum = (uint8_t)strtol(checksum_str, NULL, 16);

    // Compare the checksum against the calculated one
...
file_info_t is_valid_bin_file(const char* path_filename) {
    ...
    if (sp < 0x20000000 || sp > 0x40000000) return info;
    if (reset_vector < 0x08000000 || reset_vector > 0x20000000) return info;
...
file_info_t parse_firmware_file(const char* filename) {
    file_info_t info = { .file_type = FILE_TYPE_UNKNOWN };
	  ...
	  if(info.file_type == FILE_TYPE_HEX) return info;
			
		info = is_valid_bin_file(filename);
		if(info.file_type == FILE_TYPE_BIN) return info;
...
int parse_hex_line(const char *line, hex_record_t *record_out) {
    ...
    if (type == 0x00) 
        ...
        if (record_out) 
            ...
            for (int i = 0; i < len; ++i) {
                if (parse_hex_byte(&line[9 + i * 2], &record_out->data[i]) < 0) return -3;
            }
        }
        return 1;
    } 
		...
		{ 
        uint8_t high, low;
        if (parse_hex_byte(&line[9], &high) < 0 || parse_hex_byte(&line[11], &low) < 0) return -4;
...
file_read_status_t read_next_hex_record(FILE *hex_file,long *file_pos,hex_record_t *hex_record) {
 ...
 if (!hex_file) return FILE_READ_ERROR;
